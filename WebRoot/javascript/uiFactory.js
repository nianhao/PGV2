var uiFactory=function(){
	...
	var addProgressBar=function(id){
		...
		$("body").append(str);
		$("#"+id).progressbar({value:20});
	}
	
	var createVideoWin2=function(id){
		var str=
			'<div class="videoContent" id="'+id+'" >'
	...
	var createVideoCall=function(jsonObj){
		var str=
			 '<div class="videoCallContent" id="'+jsonObj.fromUid+'">'
		...
		$("#acceptSlider").progressbar();
	  	...
	  	$(".slider").draggable({
	  		...
	  		drag:function(){
	  			...
	  			$(this).parent().css({"background-position-x":shouldBe});
	  		},
	  		stop:function(){
	  			var now=$(this).parent().css("background-position-x");
	  			...
	  			if(nowNum<-130){
	  				$(this).css({"left":0});
	  				$(this).parent().css({"background-position-x":-260});
	  			}
	  			else{
	  				$(this).css({"left":241});
	  				$(this).parent().css({"background-position-x":-260+241});
	  				acceptRing(jsonObj);
